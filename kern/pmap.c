...
mem_init(void)
{
	...
	kern_pgdir = (pde_t *) boot_alloc(PGSIZE);                          //kern_pgdir is an virtual address
	memset(kern_pgdir, 0, PGSIZE);					                    //it seems like that the first arg of memset() is virtual address
	...
	kern_pgdir[PDX(UVPT)] = PADDR(kern_pgdir) | PTE_U | PTE_P;	        //definition of macro 'PADDR()' sees in 'kern/pmap.h'.
	                                                                    //this statement means mapping 'UVPT'(virtual address) to
	                                                                    //'PADDR(kern_pgdir)'(physical address) and set Flags
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    pte_t *result;
	if (pgdir[PDX(va)] == (pde_t)NULL) {            //yet to create
		if (create == 0)
			return NULL;
		else {
			...
			if (tmp == NULL)
				...
				result = page2kva(tmp);				                    //return an virtual address
			}
		}
	}
	else	                    
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	//test output
	//cprintf(">>  boot_map_region() was called!\n");
